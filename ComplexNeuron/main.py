            ...
            if (epoch + 1) % EVAL_EVERY == 0:
                with concurrent.futures.ProcessPoolExecutor(cpus) as executor:
                    evaluations = executor.map(
                        worker_fn, [solver.current_param() for i in range(64)]
                    )
                ...
                with open("eval_log_" + str(run) + ".txt", "a") as outfile:
                    outfile.write(
                        "EVAL:   "
                        + " mean: "
                        + str(np.mean(evaluations))
                        + " best: "
                        + str(np.max(evaluations))
                        + " worst: "
                        + str(np.min(evaluations))
                        + " std.: "
                        + str(np.std(evaluations))
                        + "\n"
                    )
